...
public class HashBasedWordCO {
	...
	public List<String> splitSentences(String text, HashSet<Character> lineEnders) {
		...
		if (preI < i)
			sentences.add(text.substring(preI, i).trim().replaceAll("[^A-Za-z0-9 ]+", "").toLowerCase(Locale.ENGLISH));
		return sentences;
	}
	...
	public HashSet<String> getUniqueWords(String text) {
		...
		for (String sentence : sentences) {
			for (String word : sentence.split("\\s+")) {
				...
				for (String specialWords : ignoreList) {
					if (specialWords.equals(word)) {
						ignoreWord = true;
						break;
					} else {
						ignoreWord = false;
				...
				if (!ignoreWord && word.length() >= minWordLen && !uWords.contains(word)) {
					uWords.put(word);
				}
	...
	public void fillCoMat(String text, String[] keyWords) {
		...
		for (String sentence : sentences) {
			...
			for (int i = 0; i < wordList.length; i++) {
				HashCounter<String> wordInHash = coMat.get(wordList[i]);
				if (wordInHash != null) {
					calculateOccurence(wordList[i], wordList, i);
				}
			}
		}
	...
	public void calculateOccurence(String word, String[] wordList, int position) {

		for (int i = position - windowSize; i <= position + windowSize; i++) {
			if (i >= 0 && i < wordList.length && words.contains(wordList[i]) && i != position) {
				coMat.get(word).increment(wordList[i]);
			}
