...
public class ArrayBasedHeap<Key, Value> implements iAdaptablePriorityQueue<Key, Value> {
	...
	/*
	 * 
	 * private int find(Key k) { int index = -1; for (int i=0;i<n;i++) {
	 * if(getComparator().compare(nodes.get(i).getKey(), k) ==0) { index=i; break; }
	 * }
	 * 
	 * return index; }
	 */
	...
	/*
	 * @Override public Key replaceKey(Entry<Key, Value> entry, Key k) {
	 * 
	 * Key replacedKey=entry.getKey(); Value v = remove(entry.getKey()); if(v==null)
	 * { return null; } insert(k,entry.getValue());
	 * 
	 * return replacedKey;
	 * 
	 * for (int i = 0; i <n ; i++) { Entry<Key,Value> temp=nodes.get(i); int
	 * compare=comparatorKey.compare(temp.getKey(),entry.getKey()); if(compare==0){
	 * Key oldK=temp.getKey(); temp.setKey(k); int pos=downheap(i); if(pos==i)
	 * upheap(i); return oldK; } } return null; }
	 * 
	 * 
	 * 
	 * 
	 * 
	 * @Override public Key replaceKey(Value v, Key k) { // TODO Auto-generated
	 * method stub for(int i=0; i<n;i++) { Entry<Key,Value> node=nodes.get(i);
	 * if(node.getValue().equals(v)) { Key oldK=node.getKey(); remove(oldK);
	 * insert(k,v); return oldK; } } return null; }
	 * 
	 * 
	 * 
	 * 
	 * @Override public Value replaceValue(Entry<Key, Value> entry, Value v) { //
	 * TODO Auto-generated method stub for(int i=0; i<n;i++) { Entry<Key,Value>
	 * node=nodes.get(i);
	 * 
	 * if(getComparator().compare(node.getKey(),entry.getKey()) == 1){ Value
	 * tempV=node.getValue(); node.setValue(v); return tempV; } } return null;
	 * 
	 * }
	 */
	...
	public Key replaceKey(Entry<Key, Value> entry, Key k) {
		// TODO Auto-generated method stub
		/*
		 * Key oldKey=entry.getKey(); Value value=entry.getValue(); Value
		 * check=remove(oldKey); if(check==null) return null; insert(k,value); return
		 * oldKey;
		 */
		/*
		for(int i=0;i<n;i++) {
		...
		*/
		...
		for (int i = 0; i < n; i++) {
			...
			if (compare == 0 && temp.getValue().equals(entry.getValue())) {
				Key oldK = temp.getKey();
				temp.setKey(k);
				int pos = downheap(i);
				if (pos == i)
