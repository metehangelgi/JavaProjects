...
public class Maze {
	...
	public boolean solveMaze(iSimpleContainer<Coordinate> sc, iDeque<Coordinate> visitedNodes, String mazeName)
		...
		ArrayDeque<Coordinate> neighbors=null;
		...
		while (!sc.isEmpty()) {
			...
			if (isExit(currentState.x, currentState.y)) {
		        ...
		        neighbors.addFront(new Coordinate(currentState.x-1,currentState.y));
		        neighbors.addFront(new Coordinate(currentState.x,currentState.y+1));
		        neighbors.addFront(new Coordinate(currentState.x+1,currentState.y));
		        neighbors.addFront(new Coordinate(currentState.x,currentState.y-1));
		        for (Coordinate neighbor : neighbors) {
		          if (isInMaze(neighbor) && (currentMaze[neighbor.x][neighbor.y] != I)) {
		              sc.push(neighbor);
	...
	private boolean isInMaze(Coordinate c) {
		return ((0 <= c.x) && (c.x < rows) && (0 <= c.y) && (c.y < cols));
	}
