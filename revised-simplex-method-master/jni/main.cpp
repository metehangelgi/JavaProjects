...
#include <string.h>
#include <assert.h>
#include "com_home_simplexmethod_SimplexMethodWrapper.h"
...
void printToJava(char * str)
{
	jstring jstr = g_env->NewStringUTF(str);
	jclass clazz = g_env->GetObjectClass(g_obj);
	jmethodID printMessage = g_env->GetMethodID(clazz, "callback", "(Ljava/lang/String;)V");
	jobject result = g_env->CallObjectMethod(g_obj, printMessage, jstr);
}

void endOfWork(char * str)
{
	jstring jstr = g_env->NewStringUTF(str);
	jclass clazz = g_env->GetObjectClass(g_obj);
	jmethodID doneCallback = g_env->GetMethodID(clazz, "doneCallback", "(Ljava/lang/String;)V");
	jobject result = g_env->CallObjectMethod(g_obj, doneCallback, jstr);
}
void sizeCallback(char * str)
{
	jstring jstr = g_env->NewStringUTF(str);
	jclass clazz = g_env->GetObjectClass(g_obj);
	jmethodID doneCallback = g_env->GetMethodID(clazz, "sizeOfResult", "(Ljava/lang/String;)V");
	jobject result = g_env->CallObjectMethod(g_obj, doneCallback, jstr);
}
...
void print_optimal_vector(Tableau *tab, char *message) {
  ...
  for(j=1;j<tab->n;j++) {
    ...
    if (xi != -1)
    {
      ...
      sprintf(tmp,"x%d=%3.2lf, ", j, tab->mat[xi][0] );
  ...
  if(strcmp(message,"Optimal vector")==0)
  {
	  endOfWork(buffer);
  }
  printf("\n");
  ...
  (JNIEnv* env, jobject obj)
{
	g_env = env;
	g_obj = obj;
	printToJava("This is test");
	jstring jstr = env->NewStringUTF("This comes from jni.");
	jclass clazz = env->GetObjectClass(obj);
    jmethodID printMessage = env->GetMethodID(clazz, "callback", "(Ljava/lang/String;)V");
    jobject result = env->CallObjectMethod(obj, printMessage, jstr);
    read_tableau(&tab, "/home/dimka/RevisedSimplexMethod/jni/file.txt");
