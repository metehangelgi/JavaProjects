...
/**
 * This class exports methods for converting from one currency to another based
 * on data from a file.
 */
...
public class CurrencyConversionTable {
	...
	/**
	 * Constructs a new <code>CurrencyConversionTable</code> from the data in
	 * the specified file. The table consists of lines in the following form:
	 * 
	 * <pre>
	 * <code>
	 *    currency=rate
	 ...
	 * <code>rate</code> is a floating-point value specifying the equivalent
	 * value of one unit of that currency in the standard currency, which is
	 * typically US Dollars, even though that is nowhere enforced in the
	 * structure.
	 * 
	 * <p>
	 * This method throws an <code>ErrorException</code> if the file cannot be
	 * found or contains data errors.
	 */
	public CurrencyConversionTable(String filename) {
		try {
			...
			while (true) {
				...
				double rate = Double.parseDouble(line.substring(equals + 1)
						.trim());
	...
	/**
	 * Returns the current rate for the specified currency name. If the name
	 * does not exist, this method throws an <code>ErrorException</code>
	 * 
	 * @return An array of the currency names in alphabetical order
	 */
