...
public class UndirectedWeightedGraph<V> extends BaseGraph<V> {
  ...
  public void insertEdge(V source, V target, float weight) {
    ...
    Edge sourceEdge=new Edge(sourceVertex,weight);
    ...
    if(!sourcesEdges.contains(targetEdge)){
      ...
      ArrayList<Edge> targetEdges=vertices.get(targetVertex);
      sourcesEdges.stream().filter(s->s.equals(targetEdge)).forEach(s->s.setWeight(weight));
      targetEdges.stream().filter(s->s.equals(sourceEdge)).forEach(s->s.setWeight(weight));
    }
  ...
  public Iterable<V> outgoingNeighbors(V v) {
    ...
    return vertices.get(vertex).stream().map(ve->ve.getVertex().getV()).collect(Collectors.toList());
  }
